...
class BoardSnapshot:
    ...
    @property
    def units_by_position(self) -> dict[tuple[int, int], tuple[UnitSnapshot, ...]]:
        cached = self._cache.get("units_by_position")
        if cached is None:
            mutable: dict[tuple[int, int], list[UnitSnapshot]] = {}
            for unit in self.units.values():
                mutable.setdefault((unit.x, unit.y), []).append(unit)
            cached = {position: tuple(units) for position, units in mutable.items()}
            self._cache["units_by_position"] = cached
        return cached  # type: ignore[return-value]

    @property
    def city_teams_by_position(self) -> dict[tuple[int, int], int]:
        cached = self._cache.get("city_teams_by_position")
        if cached is None:
            cached = {(tile.x, tile.y): tile.team for tile in self.city_tiles}
            self._cache["city_teams_by_position"] = cached
        return cached  # type: ignore[return-value]

    @property
    def city_tile_counts(self) -> dict[str, int]:
        cached = self._cache.get("city_tile_counts")
        if cached is None:
            counts: dict[str, int] = {}
            for tile in self.city_tiles:
                counts[tile.city_id] = counts.get(tile.city_id, 0) + 1
            cached = counts
            self._cache["city_tile_counts"] = cached
        return cached  # type: ignore[return-value]

    def cached_encoding(self, team: int) -> np.ndarray | None:
        cached = self._cache.get(f"encoded_team_{team}")
        return cached if isinstance(cached, np.ndarray) else None

    def store_encoding(self, team: int, features: np.ndarray) -> None:
        self._cache[f"encoded_team_{team}"] = features
...
def snapshot_from_game(game: object) -> BoardSnapshot:  # noqa: C901
    cache_key = (id(game.state), int(game.state["turn"]))
    cached = getattr(game, "_board_snapshot_cache", None)
    ...
    game._board_snapshot_cache = (cache_key, snapshot)  # noqa: SLF001
    return snapshot
...
def _fill_city_features(
    ...
    tile_count = snapshot.city_tile_counts[tile.city_id]
    base = f"{prefix}_city"
    features[FEATURE_INDEX[base], y, x] = 1
...
def encode_snapshot_teams(snapshot: BoardSnapshot) -> tuple[np.ndarray, np.ndarray]:  # noqa: C901
    """Encode both perspectives while sharing board-static feature construction."""
    cached = (snapshot.cached_encoding(0), snapshot.cached_encoding(1))
    ...
    for team in (0, 1):
        ...
        snapshot.store_encoding(team, features)
        encoded.append(features)
    return encoded[0], encoded[1]


def encode_snapshot(snapshot: BoardSnapshot, team: int) -> np.ndarray:
    cached = snapshot.cached_encoding(team)
    if cached is not None:
        return cached
    if team not in {0, 1}:
        raise ValueError("Team must be 0 or 1")
    return encode_snapshot_teams(snapshot)[team]
